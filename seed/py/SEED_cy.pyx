...
# namedtuple interface for communicating with python module
...
cdef class SEEDAlgorithmCy:
    ...
    cdef _cy_generate_key_schedule(self, np.ndarray[np.uint8_t, ndim=1] keys, Py_ssize_t rnd):
        ...
        #if self._p_keys_x1 == NULL and self._p_keys_x2 == NULL and self._p_keys_x3 == NULL and self._p_keys_x4 == NULL:
        ...
        if self._p_key_schedule_0 == NULL and self._p_key_schedule_1 == NULL:
            ...
            # allocate memory
            self._p_key_schedule_0 = <np.uint32_t *> PyMem_Malloc(self._num_keys * sizeof(np.uint32_t))
            self._p_key_schedule_1 = <np.uint32_t *> PyMem_Malloc(self._num_keys * sizeof(np.uint32_t))
            if self._p_key_schedule_0 is NULL or self._p_key_schedule_1 is NULL:
                logger.exception("PyMem_Malloc failed to allocate memory fot Key Schedule")
        #else:
            #logger.error("Key schedule memory from previous sessions is not de-allocated")

        # round 0 update
        if rnd == 0:
            for index in range(self._num_keys):
                temp_var0 = self._p_keys_x1[index] + self._p_keys_x3[index] - self._const_KC[0]
                temp_var1 = self._p_keys_x2[index] - self._p_keys_x4[index] + self._const_KC[0]
                self._p_key_schedule_0[index] = self._const_SS0[temp_var0 & 0xff] ^ \
                                                   self._const_SS1[(temp_var0>>8) & 0xff] ^ \
                                                   self._const_SS2[(temp_var0>>16) & 0xff] ^ \
                                                   self._const_SS3[(temp_var0>>24) & 0xff]
                self._p_key_schedule_1[index] = self._const_SS0[temp_var1 & 0xff] ^ \
        ...
        for index in range(self._num_keys):
            ...
            self._p_keys_x1[index] = self._p_keys_x1[index]>>8 ^ self._p_keys_x2[index]<<24
            self._p_keys_x2[index] = self._p_keys_x2[index]>>8 ^ temp_var0<<24
        if rnd == 1:
            for index in range(self._num_keys):
                temp_var0 = self._p_keys_x1[index] + self._p_keys_x3[index] - self._const_KC[1]
                temp_var1 = self._p_keys_x2[index] - self._p_keys_x4[index] + self._const_KC[1]
                self._p_key_schedule_0[index] = self._const_SS0[temp_var0 & 0xff] ^ \
                                                   self._const_SS1[(temp_var0>>8) & 0xff] ^ \
                                                   self._const_SS2[(temp_var0>>16) & 0xff] ^ \
                                                   self._const_SS3[(temp_var0>>24) & 0xff]
        ...
        for index_rnd in range(2,MAX_ROUNDS,2):
            for index in range(self._num_keys):
                ...
                self._p_keys_x3[index] = self._p_keys_x3[index]<<8 ^ self._p_keys_x4[index]>>24
                self._p_keys_x4[index] = self._p_keys_x4[index]<<8 ^ temp_var0>>24
            if rnd == index_rnd:
                for index in range(self._num_keys):
                    temp_var0 = self._p_keys_x1[index] + self._p_keys_x3[index] - self._const_KC[rnd]
                    temp_var1 = self._p_keys_x2[index] + self._const_KC[rnd] - self._p_keys_x4[index]
                    self._p_key_schedule_0[index] = self._const_SS0[temp_var0 & 0xff] ^ \
                                                       self._const_SS1[(temp_var0>>8) & 0xff] ^ \
                                                       self._const_SS2[(temp_var0>>16) & 0xff] ^ \
                                                       self._const_SS3[(temp_var0>>24) & 0xff]
    ...
    cdef _cy_fragment_keys_to_words(self, np.ndarray[np.uint8_t, ndim=1] keys):
        ...
        if self._p_keys_x1 is NULL and self._p_keys_x2 is NULL and self._p_keys_x3 is NULL \
                and self._p_keys_x4 is NULL:
            # allocate memory
            self._p_keys_x1 = <np.uint32_t *> PyMem_Malloc(self._num_keys * sizeof(np.uint32_t))
            self._p_keys_x2 = <np.uint32_t *> PyMem_Malloc(self._num_keys * sizeof(np.uint32_t))
            self._p_keys_x3 = <np.uint32_t *> PyMem_Malloc(self._num_keys * sizeof(np.uint32_t))
            self._p_keys_x4 = <np.uint32_t *> PyMem_Malloc(self._num_keys * sizeof(np.uint32_t))
            if self._p_keys_x1 is NULL or self._p_keys_x2 is NULL or self._p_keys_x3 is NULL \
                    or self._p_keys_x4 is NULL:
                logger.exception("PyMem_Malloc failed to allocate memory fot Keys")
        #else:
            #logger.error("Keys memory from previous sessions is not de-allocated")

        # byte shuffle logic to convert four bytes to unsigned word, so that arithmetic operations can be performed
        for index in range(self._num_keys):
            index_1 = index * 16
            self._p_keys_x1[index] = ((<np.uint32_t> keys[index_1]) << 24) | ((<np.uint32_t> keys[index_1+1]) << 16)\
                                   | ((<np.uint32_t> keys[index_1+2]) << 8) | (<np.uint32_t> keys[index_1+3])
            index_1 += 4
            self._p_keys_x2[index] = ((<np.uint32_t> keys[index_1]) << 24) | ((<np.uint32_t> keys[index_1+1]) << 16)\
                                   | ((<np.uint32_t> keys[index_1+2]) << 8) | (<np.uint32_t> keys[index_1+3])
            index_1 += 4
            self._p_keys_x3[index] = ((<np.uint32_t> keys[index_1]) << 24) | ((<np.uint32_t> keys[index_1+1]) << 16)\
                                   | ((<np.uint32_t> keys[index_1+2]) << 8) | (<np.uint32_t> keys[index_1+3])
            index_1 += 4
            self._p_keys_x4[index] = ((<np.uint32_t> keys[index_1]) << 24) | ((<np.uint32_t> keys[index_1+1]) << 16)\
                                   | ((<np.uint32_t> keys[index_1+2]) << 8) | (<np.uint32_t> keys[index_1+3])
